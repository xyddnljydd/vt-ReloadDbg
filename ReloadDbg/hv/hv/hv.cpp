...
static bool find_offsets() {
  ...
  DbgPrint("[hv] System EPROCESS = 0x%llX.\n",
  ...
  DbgPrint("[hv] EPROCESS::UniqueProcessId offset = 0x%llX.\n",
    ghv.eprocess_unique_process_id_offset);
...
static bool create() {
  ...
  DbgPrint("[hv] Allocated %u VCPUs (0x%llX bytes).\n", ghv.vcpu_count, arr_size);
