...
bool virtualize_cpu(vcpu* const cpu) {
  ...
  DbgPrint("[hv] Measured VM-exit overhead (TSC = %llX).\n",
	  cpu->vm_exit_tsc_overhead);
#endif
  DbgPrint("[hv] Measured VM-exit overhead (MPERF = %llX).\n",
    cpu->vm_exit_mperf_overhead);
  DbgPrint("[hv] Measured VM-exit overhead (CPU_CLK_UNHALTED.REF_TSC = %llX).\n",
