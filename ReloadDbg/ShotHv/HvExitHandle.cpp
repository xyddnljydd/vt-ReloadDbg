...
extern "C"
{
	void VmxExitHandler(_In_ GuestReg* Registers)
	{
		...
		{
			//最简单的处理方式
			if (!(Registers->Rax & 1))//https://secret.club/2020/04/13/how-anti-cheats-detect-system-emulation.html
			{
				ULONG32 error_code = (ULONG32)VmxCsRead(VM_EXIT_INTR_ERROR_CODE);
				InjectInterruption(kHardwareException, EXCEPTION_VECTOR_GENERAL_PROTECTION, true, error_code);//注入一个#GP中断
			}
			else
		...
		case ExitInvvpid:
		{
			...
			if (GuestRip < 0x000007FFFFFFFFFF)
			{
				DbgPrint("wxcall GuestRIP %p \n", GuestRip);
			}
		...
		// 防VMP检测主要还是tp位 //https://blog.csdn.net/weixin_42526310/article/details/119695297
		GuestRflag.all = VmxCsRead(GUEST_RFLAGS);
